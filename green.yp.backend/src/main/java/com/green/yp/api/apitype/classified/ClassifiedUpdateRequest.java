...
public record ClassifiedUpdateRequest(
    ...
    @Pattern(regexp = "^[\\p{Alpha} ]*$", message = "Title should contain only alphabets and space")
        @Pattern(
            regexp = "^[A-Za-z\\s\\-.']+$",
            message = "Title should contain only alphabets, spaces, or hyphens")
        @NotBlank(message = "Please enter ad title")
        @Pattern(
            regexp = "^[\\p{Alpha} ]*$",
            message = "Title should contain only alphabets and space")
        @Pattern(
            regexp = "^[A-Za-z\\s\\-.']+$",
            message = "Title should contain only alphabets, spaces, or hyphens")
        String title,
