...
public class DefaultGeocodeServiceImpl implements GeocodingService {
  ...
  public GeocodeLocation getCoordinates(String zipCode) {
    ...
    return geocodeOptional
        .map(gc -> new GeocodeLocation(gc.getLatitude(), gc.getLongitude()))
        .orElseGet(
            () -> {
              log.warn("No Geocode (lat/ long) found for zip code {}", zipCode);
              return new GeocodeLocation(
                  BigDecimal.valueOf(33.771759), BigDecimal.valueOf(-84.367154));
            });
  }
  ...
  public GeocodeLocation getCoordinates(String address, String city, String state, String zip) {
    ...
    Optional<PostalCodeGeocode> geocodeOptional =
        repository.findFirstByPlaceNameAndState(city, state);
    return geocodeOptional
        .map(gc -> new GeocodeLocation(gc.getLatitude(), gc.getLongitude()))
        .orElseGet(
            () -> {
