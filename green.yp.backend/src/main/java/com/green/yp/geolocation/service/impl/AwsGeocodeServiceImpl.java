...
public class AwsGeocodeServiceImpl implements LiveGeocodeService {
  ...
  public AwsGeocodeServiceImpl(
      LocationClient awsLocationClient, PostalCodeGeocodeRepository repository) {
    ...
    try {
      ...
      if (!response.results().isEmpty()) {
        ...
        return new GeocodeLocation(
            BigDecimal.valueOf(position.get(1)), BigDecimal.valueOf(position.get(0)));
      }
