...
public class ProducerUserService {
    ...
    public ProducerUserService(ProducerOrchestrationService producerService,
                               ProducerAuthUserMapper authUserMapper,
                               ProducerContactService contactService,
                               ProducerUserCredentialsRepository credentialsRepository,
                               InvalidCredentialsCounterRepository credentialsCounterRepository,
    ...
    ProducerCredentialsResponse createCredentials(
        ...
        AuthServiceResponse<RegistrationResponse> response = authenticationContract
                .registerUser(producerId, accountMaster, credentialsRequest);
        ...
        authorizedUser.setExternalAuthorizationServiceRef(response.getActualResponse().user.id.toString());
    ...
    public void disableAuthentication(@NonNull @NotNull UUID contactId) {
        ...
        credentialsRepository.findContactCredentials(contactId)
                .stream()
                .forEach(creds -> {
    ...
    public ResponseApi<List<ProducerUserResponse>> findUsers(UUID producerId, String firstName, String lastName) {
        return null;
    }
    ...
    public ProducerCredentialsResponse findMasterUserCredentials(UUID producerId) {
        ...
        return credentialsRepository.findMasterUserCredentials(producerId)
                .map(authUserMapper::fromEntity)
                .orElseThrow(() -> new NotFoundException("Master user admin credentials not found"));
    }

    @Transactional
    public ProducerCredentialsResponse updateUserCredentials(UserCredentialsRequest userCredentialsRequest,
                                                             UUID producerId,
                                                             UUID credentialsId,
                                                             String ipAddress) throws NoSuchAlgorithmException {
        log.info("Updating existing user credentials for producer {} and contact {}",
                producerId, userCredentialsRequest.producerContactId());

        ProducerUserCredentials credentials = credentialsRepository.findById(credentialsId)
                .orElseThrow(() -> {
        ...
        AuthServiceResponse<UserResponse> response = authenticationContract.findUser(credentials.getExternalAuthorizationServiceRef());
        if ( !response.getActualResponse().user.active){
            log.error("Cannot modify disabled fusionAuth credentials for credentailsId {}", credentialsId);
            throw new PreconditionFailedException("Cannot update / modify disabled credentials");
        }

        AuthServiceResponse<UserResponse> updatedUser = authenticationContract.modifyUser(producerId,
                credentials.getExternalAuthorizationServiceRef(),
    ...
    @Transactional
    public ProducerCredentialsResponse replaceMasterUserCredentials(@NotNull @NonNull UserCredentialsRequest masterUserCredentials,
                                                                    @NotNull @NonNull UUID producerId,
                                                                    @NotNull @NonNull String ipAddress) throws NoSuchAlgorithmException {
        ...
        if ( masterUserCredentials.producerContactId() == null ){
            contactId = contactService.findActiveContact(masterUserCredentials.producerContactId()).getId();
        } else {
            contactId = contactService.findAdminContacts(producerId)
                    .stream()
                    .filter( contact -> contact.producerContactType() == ProducerContactType.PRIMARY
                                && contact.emailAddress().equals(masterUserCredentials.emailAddress()))
                    .findFirst()
                    .orElseThrow(() -> {
                        log.error("No {} contact for subscriber {}", ProducerContactType.PRIMARY, producerId);
                        return new NotFoundException("Cannot find PRIMARY contact for subscriber");
                    }).contactId();
        }

        ProducerUserCredentials credentials = credentialsRepository.findMasterUserCredentials(producerId)
                .orElseThrow(() -> {
        ...
        return createCredentials(masterUserCredentials,
                true,
...
//            AuditActionType.DELETE_LOCATION)
    public void removeCredentials(List<UUID> producerIdList) {
        List<ProducerUserCredentials> credentials = credentialsRepository.findCredentials(producerIdList);
        credentials
                .stream()
                .filter(creds -> StringUtils.isNotBlank(creds.getExternalAuthorizationServiceRef()))
                .forEach(creds -> authenticationContract.removeUser(creds.getExternalAuthorizationServiceRef()));
