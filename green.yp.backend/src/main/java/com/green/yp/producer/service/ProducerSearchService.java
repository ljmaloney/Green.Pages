...
public class ProducerSearchService {
  ...
  public void activateProducer(UUID producerId) {
    ...
    locationRepository
        ...
        .ifPresentOrElse(
            loc -> createLocationSearchRecords(loc.getId()),
            () -> {
  ...
  public void updateProducer(ProducerResponse producerResponse) {
    ...
    List<ProducerLocation> locations =
        locationRepository.findActiveLocations(producerResponse.producerId(), true);

    List<SearchMasterRequest> searchRequests =
  ...
  @Async("threadPoolSearchTaskExecutor")
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public void upsertProducerLocation(
      @NotNull @NonNull UUID producerId,
      @NotNull @NonNull ProducerLocationResponse locationResponse,
      String requestIP) {
    log.info(
        "Upserting producer location search records producerId: {} locationId: {}",
        producerId,
        locationResponse.locationId());
    searchContract.upsertSearchMaster(
        createSearchRequests(locationResponse.locationId()), producerId);

    searchContract.updateLocation(producerSearchMapper.toSearchLocationUpdate(locationResponse));
  }

  public void deleteSearch(
      @NotNull @NonNull UUID serviceId, @NotNull @NonNull SearchRecordType recordType) {
  ...
  public void upsertProducerService(ProducerServiceResponse response) {
    ...
    var lob =
        lobContract.findLineOfBusiness(
            profile.producer().getPrimaryLineOfBusiness().getLineOfBusinessId());
    String keywordsBuilder = lob.lineOfBusinessName() + " " + response.shortDescription();
    searchContract.upsertSearchMaster(
        List.of(
            producerSearchMapper.toSearchMaster(
                response,
                profile.producer(),
                profile.location(),
                profile.contact(),
  ...
  public void upsertProduct(ProducerProductResponse response) {
    ...
    var lob =
        lobContract.findLineOfBusiness(
            profile.producer().getPrimaryLineOfBusiness().getLineOfBusinessId());
    String keywordsBuilder = lob.lineOfBusinessName() + " " + response.name();
    var productList =
        List.of(
            producerSearchMapper.toSearchMaster(
                response,
  ...
  private ProducerSearchRecord getProducerProfile(UUID locationId) {
    var profile =
        searchRepository
            .findProducerProfile(locationId)
            .orElseThrow(
                () -> {
                  log.error("No producer profile found for location {}", locationId);
                  return new IllegalStateException(
                      "No producer profile found for location " + locationId);
                });
  ...
  private static void validateNotCancelled(UUID locationId, ProducerSearchRecord profile) {
    if (profile.producer().getCancelDate() != null
      ...
      log.error(
          "Error creating search master record for  location {} - cancelDate {} is in the past",
          locationId,
          profile.producer().getCancelDate());
      throw new PreconditionFailedException(
          "Error creating search master record for location, producer cancelled " + locationId);
    }
  }
  ...
  private String createProfileKeywords(String keywords, LineOfBusinessDto lob) {
    ...
    final StringBuilder keywordsBuilder =
        new StringBuilder(keywords).append(" ").append(lob.lineOfBusinessName()).append(" ");
