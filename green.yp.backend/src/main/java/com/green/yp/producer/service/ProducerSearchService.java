...
public class ProducerSearchService {
  ...
  public ProducerSearchService(
          ProducerSearchRepository searchRepository, ProducerLocationService producerLocationService,
  ...
  public PageableResponse<ProducerSearchResponse> search(
    ...
    List<SearchMasterRequest> searchList =
        profile.producer().getLinesOfBusiness().parallelStream()
            .map(
                pline -> {
                  var lob = lobContract.findLineOfBusiness(pline.getLineOfBusinessId());
                  return producerSearchMapper.toSearchMaster(profile.producer(),
                          profile.location(),
                          profile.contact(),
                          lob,
