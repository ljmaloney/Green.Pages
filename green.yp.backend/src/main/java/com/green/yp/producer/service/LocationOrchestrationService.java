...
public class LocationOrchestrationService {
  ...
  public ProducerLocationResponse createLocation(
      @Valid CreateLocationRequest createRequest, UUID producerId, String requestIP) {
    log.info(
        "Creating location {} record for producer: {}",
        createRequest.locationRequest().locationName(),
        producerId);
    var locationResponse = locationService.createLocation(createRequest, producerId, requestIP);

    Optional.ofNullable(createRequest.contactRequest())
        .ifPresent(
            contactRequest ->
                contactService.createContact(
                    contactRequest, producerId, locationResponse.locationId(), requestIP));

    searchService.upsertProducerLocation(
        locationResponse.producerId(), locationResponse, requestIP);
