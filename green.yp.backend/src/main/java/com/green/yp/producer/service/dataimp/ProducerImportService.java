...
public class ProducerImportService {
    ...
    @Transactional
    public UUID importProducer(UUID lineOfBusinessId, ProducerCsvRecord importRecord) throws Exception{
            var location = geocodingService.getCoordinates(importRecord.address, importRecord.city, importRecord.state, importRecord.zip);

            // Use contact as company name if company is empty
            String businessName =
                    StringUtils.getIfBlank(importRecord.company, () -> importRecord.contact);
