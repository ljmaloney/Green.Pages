...
public class CacheConfig {
  ...
  public long getExpireAfterWrite(CacheEnumeration cache) {
    if (configs.containsKey(cache.cacheName)) {
      return configs.get(cache.cacheName).expirationMinutes != null
          ? configs.get(cache.cacheName).expirationMinutes
          : cache.expirationMinutes;
    }
  ...
  public long getIdleScanTime(CacheEnumeration cache) {
    if (configs.containsKey(cache.cacheName)) {
      return configs.get(cache.cacheName).getIdleScanTime() != null
          ? configs.get(cache.cacheName).getIdleScanTime()
          : cache.getIdleScanTime();
    }
  ...
  public boolean getEternal(CacheEnumeration cache) {
    if (configs.containsKey(cache.cacheName)) {
      return configs.get(cache.cacheName).getEternal() != null
          ? configs.get(cache.cacheName).getEternal()
          : cache.isEternal();
    }
  ...
  public boolean permitNullValues(CacheEnumeration cache) {
    if (configs.containsKey(cache.cacheName)) {
      return configs.get(cache.cacheName).permitNullValues() != null
          ? configs.get(cache.cacheName).permitNullValues()
          : cache.isPermitNullValues();
    }
