...
public final class XSSRequestWrapper extends HttpServletRequestWrapper {
  ...
  public ServletInputStream getInputStream() throws IOException {
    if (super.getContentType().equalsIgnoreCase("application/json")) {
      ...
      return new BufferedXssJsonInputStream(super.getInputStream(), XSSRequestWrapper::cleanXSS);
    }
  ...
  public Map<String, String[]> getParameterMap() {
    ...
    return parameterMap.entrySet().stream()
        .collect(
            Collectors.toUnmodifiableMap(Map.Entry::getKey, entry -> getArray(entry.getValue())));
  }
