...
/**
 * Provides a Buffered ServletInputStream for the purpose of sanitization of JSON
 * documents.
...
public class BufferedXssJsonInputStream extends ServletInputStream {
    ...
    public BufferedXssJsonInputStream(ServletInputStream inputStream,
                                      Function<String, String> xssEscapeFunction) throws IOException {
        this(inputStream, 512,
                xssEscapeFunction != null ? xssEscapeFunction : BufferedXssJsonInputStream::cleanXSS);
    }

    public BufferedXssJsonInputStream(ServletInputStream inputStream, int contentSize,
                                      Function<String, String> xssEscapeFunction) throws IOException {
    ...
    private void sanitizeInput(ServletInputStream inputStream, int contentSize) throws IOException {
        ...
        while ((ch = inputStream.read()) != -1) {
            if (JSON_STRING_DELIMITER == (char) ch && !foundString) { //test for start delimiter
                ...
                continue;
            } else if (JSON_STRING_DELIMITER == (char) ch && '\\' != (char) prevCh && foundString) { //test for end delimiter
